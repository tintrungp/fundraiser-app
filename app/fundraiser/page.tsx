...
const MISSION_COLORS = [
  { from: 'from-[#FF6B6B]', to: 'to-[#4ECDC4]', accent: 'text-[#FF6B6B]', hoverBorder: 'hover:border-[#FF6B6B]' },
  { from: 'from-[#4ECDC4]', to: 'to-[#FF6B6B]', accent: 'text-[#4ECDC4]', hoverBorder: 'hover:border-[#4ECDC4]' },
];
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
      ...
      <div className="text-center mb-10 sm:mb-16">
        <div className="inline-flex items-center gap-2 mb-4">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900">{title}</h1>
        </div>
        <p className="text-base sm:text-lg text-gray-500 max-w-2xl mx-auto">{description}</p>
      </div>
      ...
      <div className="mb-10 sm:mb-12">
        <div className="flex items-center gap-2 mb-6 sm:mb-8">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Progress</h2>
        </div>
          ...
          {missions.map((mission, index) => {
            ...
            return (
              <a
                ...
                className={`group block bg-white border-2 border-gray-200 ${colors.hoverBorder} rounded-3xl p-6 sm:p-8 shadow-sm hover:shadow-md transition-all duration-300`}
              >
                <div className="flex items-start justify-between mb-4 sm:mb-5">
                  <h3 className={`text-lg sm:text-xl font-bold ${colors.accent}`}>{mission.name}</h3>
                ...
                <div className="relative w-full bg-gray-100 rounded-full h-4 sm:h-5 overflow-hidden mb-3">
                  <div
                ...
                <div className="flex justify-between items-center">
                  ...
                  <p className="text-xs sm:text-sm text-gray-400">{formatCurrency(mission.goal - mission.current)} to go!</p>
                </div>
              </a>
            );
          })}
      ...
      <div className="text-center text-gray-300 text-lg mb-0">‿︵‿︵‿︵‿︵‿︵‿</div>
      ...
      <div className="mb-10 sm:mb-16 mt-8 sm:mt-12">
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
          {donationLinks.map((link, index) => {
            const colors = [
              { bg: 'bg-gray-100', hover: 'hover:border-gray-400' },
            ...
            return (
              <div key={link.platform} className="relative">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`block bg-white border-2 ${hasBadge ? 'border-[#FF6B6B]' : 'border-gray-200'} ${color.hover} rounded-2xl p-5 sm:p-6 hover:shadow-lg transition-all duration-300 group h-full ${hasBadge ? 'pt-7 sm:pt-8' : ''}`}
                >
                  ...
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">{link.platform}</h3>
          ...
          <div className="space-y-4 sm:space-y-6">
            {updates.map((update, index) => {
              const tagStyle = update.tag
                ? (TAG_STYLES[update.tag] ?? { bg: 'bg-gray-200', text: 'text-gray-900' })
                : null;
              return (
                <div
                  key={index}
                  className="bg-white border-2 border-gray-200 rounded-2xl p-5 sm:p-8 hover:border-[#4ECDC4] transition-colors"
                >
                  ...
                  <p className="text-gray-700 text-base sm:text-lg leading-relaxed">{update.message}</p>
                </div>
              );
            })}
          </div>
